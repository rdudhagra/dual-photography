...
def main(k):
    ...
    light_source_pattern = np.zeros((CAMERA_H, CAMERA_W, 3))
    light_source_pattern[:, k : k + 50, :] = prime_image[:, k : k + 50, :]

    prime_image_colvec = np.reshape(light_source_pattern, (-1, 3))
    ...
    if np.max(dual_image):
        dual_image = dual_image / np.max(dual_image)

    # Save dual image
    dual_image = dual_image ** (1 / 3)
    dual_image = np.clip(dual_image, 0, 1)
    skimage.io.imsave(f"frames/dual_image_{k}.jpg", dual_image)


if __name__ == "__main__":
    p = Pool()
    p.map(main, range(250, 950, 50))
